...
if(empty($_POST['cmd'])){
    foreach($dbd->query("SELECT D.image, A.nom, D.id, D.album, C.categorie FROM disques D, artistes A, categories C WHERE A.id = D.artiste AND C.id = D.categorie")as $values){
        $contenu.= '<div class="cards"><a href="index.php?page=details&id='.$values['id'].'" class="text-center"><img src="assets/images/'.$values['image'].'" alt=""><br><p class="text-center">'.$values['album']." <br>de(s)<br> ".$values['nom'].'</p></a><a href="index.php?page=reserv&id='.$values['id'].'"><button >Reserver</button></a></div>';
    }
}else{
    $cmd = $_POST['cmd'];
    switch ($cmd){
